...
for (let i = 0; i < donations.length; i++) {
	...
	const { error } = await supabase
		.from('donations')
		.update({ standard_cause_array: standardizeCauseAreas(donations[i].cause_area_array) })
	...
	console.log(
		...
		' to ',
		standardizeCauseAreas(donations[i].cause_area_array),
		' for ',
